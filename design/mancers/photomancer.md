...
**Temperature Effects:** **+5 temperature** to each unit hit (radiant warmth from the focused light beam — not enough to threaten OVERHEATED on its own, but meaningful for pushing units already near thermal thresholds).
...
**Temperature Effects:** **+5 temperature** to all units in the flash radius (brief intense light exposure — the same mild radiant warming as Light Beam, applied to all units caught in the AoE).
...
**Temperature Effects:** **creates ILLUMINATED terrain** — units standing on ILLUMINATED_GROUND tiles gain **+3 temperature per turn** from the sustained warm light. Three turns of standing on an ILLUMINATED tile = +9 temperature — not fast, but persistent. Combined with a Pyromancer on the field, this can tip enemies from WARM to HOT without the Pyromancer spending any AP.
...
**Temperature Effects:** **+15 temperature** to all units hit (intense concentrated sunlight — the highest single-spell temperature application in the Photomancer's kit). This is significant at thresholds: units already at WARM (+1 to +30) are pushed to HOT (+31 to +60) with one Sunburst hit; units already at HOT can be pushed to OVERHEATED (≥+61). See Temperature Interaction Notes below for undead and FROZEN SOLID interactions.
...
**Light = radiant energy = mild warmth.** Light spells carry +5 temperature per hit — not enough to threaten OVERHEATED by themselves, but meaningful for pushing units already near thresholds. Sunburst is the exception at +15 temperature, capable of tipping units across multiple threshold boundaries in a single cast.

- **Sunburst anti-undead + temperature:** The existing +50% damage bonus to undead from Sunburst also applies +15 temperature. Undead units (Risen Shamblers, Abyssal Horrors) typically start at -20 temperature; Sunburst pushes them to -5 (still COLD but no longer benefiting from the cold threshold bonus). Multiple Sunbursts can push undead to WARM (+10) — at which point fire spells deal +10% damage to them. This creates a Photomancer → Pyromancer anti-undead combo chain.

- **FROZEN SOLID + Sunburst = Instant Thaw:** A unit at FROZEN SOLID (≤-61) hit by Sunburst (+15 temperature) moves to approximately -46 — still SUPERCOOLED but no longer at the FROZEN SOLID threshold. The FROZEN status is removed. This makes the Photomancer the fastest way to thaw a frozen ally — a valuable support use in mixed-temperature encounters where a Cryomancer ally has accidentally frozen a friendly or a Cryomancer opponent has locked down a key unit.

- **ILLUMINATED terrain as temperature field:** Illuminate creates +3 temperature per turn to units standing on ILLUMINATED_GROUND tiles. The effect is persistent: an enemy forced to hold position on an ILLUMINATED tile (by Gravity Well, ROOTED, or tactical necessity) will warm steadily without the Photomancer spending further AP. Combined with Pyromancer fire spells already applying heat, the Photomancer's terrain can tip enemies from WARM to HOT passively.

- **Light refraction through Crystal Prism:** When Light Beam refracts through a Crystalomancer's Crystal Prism, each refracted beam segment also carries the +5 temperature to units it hits. A properly set up Crystal Prism array can warm multiple units simultaneously with a single Light Beam cast.
