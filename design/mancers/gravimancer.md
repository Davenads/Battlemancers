...
**Temperature Effects:** **0 temperature** (gravitational force does not generate heat at the tactical scale — the bolt is pure force application).
...
**Temperature Effects:** **0 temperature** (movement through gravitational force generates no heat — the displacement is instantaneous with no friction component at the tactical scale). However, units pulled across thermally hazardous terrain (ON_FIRE tiles, FROZEN tiles) are exposed to those tiles' temperature effects mid-pull.
...
**Temperature Effects:** **0 temperature** (gravity field itself generates no heat). However, the Gravity Well's pull effect moves units toward its center — if the center tile or path contains thermally active terrain (ON_FIRE, FROZEN), pulled units are exposed to those temperature effects as they move. See Temperature Interaction Notes below for fall damage and HEAVY + BURNING terrain interactions.
...
**Temperature Effects:** **+10 temperature** to the primary target (extreme compressive force generates heat — like squeezing material generates warmth; the gravitational compression is intense enough to cause mild thermal output). Splash targets take no temperature increase (the compression is localized to the primary target). A unit already at HOT (+31 to +60) hit by Crush is pushed to OVERHEATED (≥+61), adding BURNING DoT on top of the Crush damage.
...
**Gravity = force = no direct heat generation at the tactical scale.** Most Gravimancer spells are thermally neutral. However, fall damage creates trauma heat, and Crush's compressive force generates meaningful warmth. These interactions compound with the Gravimancer's displacement toolkit to create secondary temperature pressure.

- **Fall damage + temperature:** When a unit is displaced off elevated terrain (by Aeromancer wind push, Gravimancer Pull, or Gravity Well drag), the impact generates heat. Units that take fall damage also gain **+5 temperature per tile fallen** (e.g., falling 2 tiles = +10 temperature). This creates a compounding status: Gravimancer applies HEAVY (doubles fall damage), Aeromancer pushes off elevation → the unit takes double fall damage AND double temperature gain (+10 per tile fallen while HEAVY). A 3-tile fall on a HEAVY unit = 48 HP fall damage AND +30 temperature — pushing most units from neutral directly into HOT or OVERHEATED.

- **Gravitational Collapse + temperature:** The Gravitational Collapse signature pulls all units in a 5-tile radius toward the center. If any of those units were standing on BURNING terrain, they carry their accumulated OVERHEATED status to the center point. Suddenly multiple enemies are clustered at the center tile, all OVERHEATED, and a Pyromancer can hit the cluster with Conflagration Wave. The Collapse creates optimal multi-target DoT positioning — the Gravimancer does not need the enemies to be thermally charged beforehand, but if they already are, the clustering compounds the damage.

- **FROZEN SOLID + Crush:** A FROZEN SOLID unit (≤-61) hit by Crush (+10 temperature) moves from ≤-61 to approximately -51 — still SUPERCOOLED but no longer at the FROZEN SOLID threshold. The FROZEN status is removed. This can be used to "unfreeze" an enemy unit right before the Osteomancer applies Bone Shard with the BRITTLE modifier — SUPERCOOLED (-31 to -60) grants BRITTLE (+50% physical damage), so the unit is now vulnerable to physical BRITTLE damage while no longer being FROZEN and thus mobile. Often more valuable than waiting for a full Shatter hit: the unit can now move (potentially into worse positioning) and takes BRITTLE-amplified physical hits.

- **HEAVY + BURNING terrain:** A unit with HEAVY status (move range effectively limited by reluctance to trigger displacement) trying to escape BURNING terrain is constrained. HEAVY itself does not reduce move range, but paired with Gravity Well (which reduces available movement by requiring 2 AP to leave the center tile), the unit may not be able to move far enough to leave a BURNING zone in one activation. This creates an area-denial combo: Gravity Well holds units near a BURNING terrain center tile → they accumulate +10 temperature per turn from BURNING terrain passively.
