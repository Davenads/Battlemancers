...
|---|---|---|---|---|
| **Normal (GROUND)** | Fire ignites the tile normally | `ON_FIRE` | Takes spell damage + `BURNING` | None; ON_FIRE spreads each turn |
| **WET** | Fire meets moisture — water flash-evaporates | `STEAM_CLOUD` (2 turns; replaces WET state; tile is no longer ON_FIRE) | Takes spell damage + `BLINDED` (2 turns) from steam; no BURNING applied | STEAM_CLOUD occupies the tile; blocks vision in and out; adjacent units take 3 heat dmg/turn while cloud is active |
